...
class FairEmbeddings(nn.Module):
    def __init__(
        ...
        self.token_embedding = nn.Embedding(
            len(fair_tokens) + 1, token_embedding.embedding_dim
        ).to(dtype)
        self.token_embedding.weight.data.zero_()
        self.token_embedding.weight.data[1:, :] = token_embedding.weight.data[
            fair_tokens
        ]
    ...
    def forward(
        ...
        input_ids_fair = (
            input_ids.cpu().apply_(self.fair_token_id_map.get).to(input_ids.device)
        )
...
def expand_tokenizer(tokenizer, text_encoder, placeholder_token):
    ...
    for idx, random_idx in itertools.zip_longest(
        placeholder_token_id, random_token_idx
    ):
...
def text_model_forward(
    ...
    output_attentions = (
        ...
        else text_encoder.text_model.config.output_attentions
    )
    output_hidden_states = (
        output_hidden_states
        if output_hidden_states is not None
        else text_encoder.text_model.config.output_hidden_states
    )
    return_dict = (
        return_dict
        if return_dict is not None
        else text_encoder.text_model.config.use_return_dict
    )
...
def generate_image_w_prefix_embedding(
    prompt,
    noises,
    prefix_embedding,
    tokenizer,
    ...
    token_embeds = text_encoder.text_model.embeddings(
        input_ids=prompts_token["input_ids"], position_ids=None
    )
...
def main(args):
    ...
    if args.load_text_encoder_lora_from:
        ...
        text_encoder_lora_dict = torch.load(
            args.load_text_encoder_lora_from, map_location=args.device
        )
    ...
    if args.load_unet_lora_from:
        ...
        unet.load_state_dict(torch.load(args.load_debias_unet))

    if args.load_prefix_embedding_from:
        prefix_tokens = [
            f"<common-token{i+1}>" for i in range(args.number_prefix_tokens)
        ]
        ...
        prefix_embedding_dict = torch.load(
            args.load_prefix_embedding_from, map_location=args.device
        )
    ...
    test_prompts = experiment_data["test_for_midu"]
    ...
    for prompt in test_prompts:
        ...
        for i in range(args.num_imgs_per_prompt):
            hash_seed = int(hashlib.md5(prompt.encode()).hexdigest(), 16) % 100000
            torch.manual_seed(args.random_seed + i)
            noise_single = torch.randn(
                [1, 4, 64, 64], dtype=weight_dtype_high_precision
            ).to(args.device)
