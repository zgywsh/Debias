...
def plot_in_grid_gender_race(
    ...
    idxs_male_white = (
        ((preds_gender == 1) * (preds_race == 0)).nonzero(as_tuple=False).view([-1])
    )
    ...
    idxs_male_black = (
        ((preds_gender == 1) * (preds_race == 1)).nonzero(as_tuple=False).view([-1])
    )
    ...
    idxs_male_indian = (
        ((preds_gender == 1) * (preds_race == 2)).nonzero(as_tuple=False).view([-1])
    )
    ...
    idxs_male_asian = (
        ((preds_gender == 1) * (preds_race == 3)).nonzero(as_tuple=False).view([-1])
    )
    ...
    idxs_female_white = (
        ((preds_gender == 0) * (preds_race == 0)).nonzero(as_tuple=False).view([-1])
    )
    ...
    idxs_female_black = (
        ((preds_gender == 0) * (preds_race == 1)).nonzero(as_tuple=False).view([-1])
    )
    ...
    idxs_female_indian = (
        ((preds_gender == 0) * (preds_race == 2)).nonzero(as_tuple=False).view([-1])
    )
    probs_female_indian = pred_class_probs_race[idxs_female_indian]
    idxs_female_indian = idxs_female_indian[
        probs_female_indian.argsort(descending=True)
    ]

    idxs_female_asian = (
        ((preds_gender == 0) * (preds_race == 3)).nonzero(as_tuple=False).view([-1])
    )
    ...
    images_to_plot = []
    idxs_reordered = torch.torch.cat(
        [
            # idxs_male_white,
            # idxs_male_black,
            # idxs_male_indian,
    ...
    for idx in idxs_reordered:
        ...
        #     img_pil_draw._image, border=(50, 0, 0, 0), fill=race_border_color
        # )
        # img_pil_draw = ImageDraw.Draw(img_pil)
        # if pred_class_prob_race.item() < 1:
        # img_pil_draw.rectangle(
        #     [(0, 0), (50, (1 - pred_class_prob_race.item()) * 512)],
        #     fill="white",
        #     outline=None,
        # )

        img_pil = ImageOps.expand(
            img_pil_draw._image, border=20, fill=race_border_color
        )
        img_pil_draw = ImageDraw.Draw(img_pil)
        ...
        fnt = ImageFont.truetype(
            font="/data/0-utils/arial-bold.ttf",
            size=100,
        )
...
def plot_in_grid_gender_race_age(
    images,
    save_to,
    face_indicators=None,
    face_bboxs=None,
    ...
    for g in [1, 0]:
        for r in [0, 1, 2, 3]:
            for a in [0, 1]:
                idxs_ = (
                    ((preds_gender == g) * (preds_race == r) * (preds_age == a))
    ...
    for idx in idxs_reordered:
        ...
        img_pil = ImageOps.expand(
            img_pil_draw._image, border=(50, 0, 0, 0), fill=age_border_color
        )
        img_pil_draw = ImageDraw.Draw(img_pil)
        if pred_class_prob_race.item() < 1:
            img_pil_draw.rectangle(
                [(0, 0), (50, (1 - pred_class_prob_age.item()) * 512)],
                fill="white",
                outline=None,
            )
        ...
        img_pil = ImageOps.expand(
            img_pil_draw._image, border=(50, 0, 0, 0), fill=gender_border_color
        )
        img_pil_draw = ImageDraw.Draw(img_pil)
        if pred_class_prob_gender.item() < 1:
            img_pil_draw.rectangle(
...
def get_face(
    ...
    (
        ...
        images, face_app, size_face, size_aligned_face, fill_value=fill_value
    )
    ...
    for idx, image_np in enumerate(images_np):
        ...
        if len(faces_from_app) == 0:
            ...
            aligned_face_chips_app.append(
                torch.ones(
                    [1, 3, size_aligned_face, size_aligned_face],
                    dtype=images.dtype,
                    device=images.device,
                )
                * (fill_value)
...
def get_face_FR(images, face_recognition, size_face, size_aligned_face, fill_value=-1):
    ...
    for idx, image_np in enumerate(images_np):
        faces_from_FR = face_recognition.face_locations(
            image_np, model="cnn", number_of_times_to_upsample=0
        )
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--generated_imgs_dir",
        type=str,
        default="/outputs/a photo of a doctor",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
    ...
    parser.add_argument(
        "--size_face", help="size for extracted face", type=int, default=224
    )
    parser.add_argument(
        "--size_aligned_face",
...
def main(args):
    ...
    age_classifier = mobilenet_v3_large(
        weights=MobileNet_V3_Large_Weights.DEFAULT,
        width_mult=1.0,
        reduced_tail=False,
        dilated=False,
    ...
    for prompt_folder in prompt_folders:
        ...
        for img_path in tqdm(img_paths):
            ...
            face_indicator, face_bbox, face_chip, face_landmark, aligned_face_chip = (
                get_face(
                    img,
                    face_app,
                    face_recognition,
                    args.size_face,
                    args.size_aligned_face,
                )
            )
...
python eval-generated-images.py \
    --generated_imgs_dir /outputs/gen_images/images_debias \
    --save_dir /outputs/gen_images/images_debias
